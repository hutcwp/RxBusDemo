package com.hutcwp.compiler.util;

import com.hutcwp.compiler.EventProcessor;
...
public class Utils {
     ...
     * 例如:com.yy.android.sniper.annotation.mvp.LifeData<java.lang.String>
     *
     * @param typeMirror
     * @return
     */
    public static TypeName getClassNameByTypeMirror(TypeMirror typeMirror) {
        if (typeMirror.getKind().isPrimitive()) {
            ...
            if (parameterized.length == 1) {
                return ParameterizedTypeName.get(ClassName.bestGuess(className), ClassName.bestGuess(parameterized[0]));
            } else if (parameterized.length == 2) {
                return ParameterizedTypeName.get(ClassName.bestGuess(className), ClassName.bestGuess(parameterized[0]), ClassName.bestGuess(parameterized[1]));
            } else {
     ...
     * @return
     */
    public static boolean isInherit(EventProcessor processor, Element element1, Element element2) {
     ...
     * @return
     */
    public static boolean isOverrideMethod(EventProcessor processor, ExecutableElement element1, ExecutableElement element2) {
        boolean isSubsignature = processor.mTypes.isSubsignature((ExecutableType) element1.asType(), (ExecutableType) element2.asType());
    ...
    public static void error(String error, Messager messager) {
        ...
        dump.append("---------------------------------sniper apt compile fail------------------------------------\n");
        dump.append(error);
        dump.append("\n");
        dump.append("------------------------------------ reason dump end----------------------------------------\n");
